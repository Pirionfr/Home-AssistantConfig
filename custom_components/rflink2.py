...
DATA_DEVICE_REGISTER = 'rflink2_device_register'
DATA_ENTITY_LOOKUP = 'rflink2_entity_lookup'
DATA_ENTITY_GROUP_LOOKUP = 'rflink2_entity_group_only_lookup'
...
RFLINK2_GROUP_COMMANDS = ['allon', 'alloff']
...
def async_setup(hass, config):
    ...
    def async_send_command(call):
        """Send Rflink command."""
        _LOGGER.debug('Rflink2 command for %s', str(call.data))
        if not (yield from Rflink2Command.send_command(
    ...
    def event_callback(event):
        """Handle incoming Rflink2 events.

        Rflink2 events arrive as dictionaries of varying content
        ...
        is_group_event = (event_type == EVENT_KEY_COMMAND and
                          event[EVENT_KEY_COMMAND] in RFLINK2_GROUP_COMMANDS)
        if is_group_event:
    ...
    def reconnect(exc=None):
        ...
        Rflink2Command.set_rflink2_protocol(None)

        # If HA is not stopping, initiate new connection
        if hass.state != CoreState.stopping:
            _LOGGER.warning('disconnected from Rflink2, reconnecting')
            hass.async_add_job(connect)

    @asyncio.coroutine
    def connect():
        """Set up connection and hook it into HA for reconnect/shutdown."""
        _LOGGER.info('Initiating Rflink2 connection')

        # Rflink2 create_rflink_connection decides based on the value of host
        # (string or None) if serial or tcp mode should be used

        # Initiate serial/tcp connection to Rflink2 gateway
        ...
        try:
            ...
            _LOGGER.exception(
                "Error connecting to Rflink2, reconnecting in %s",
                reconnect_interval)
            hass.loop.call_later(reconnect_interval, reconnect, exc)
            return

        # Bind protocol to command class to allow entities to send commands
        Rflink2Command.set_rflink2_protocol(
            protocol, config[DOMAIN][CONF_WAIT_FOR_ACK])

        # handle shutdown of Rflink2 asyncio transport
        hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP,
                                   lambda x: transport.close())

        _LOGGER.info('Connected to Rflink2')

    hass.async_add_job(connect)
    return True


class Rflink2Device(Entity):
    """Representation of a Rflink2 device.

    Contains the common logic for Rflink2 entities.
    """
    ...
    def __init__(self, device_id, hass, name=None, aliases=None, group=True,
        ...
        # Rflink2 specific attributes for every component type
...
class Rflink2Command(Rflink2Device):
    """Singleton class to make Rflink2 command interface available to entities.

    This class is to be inherited by every Entity class that is actionable
    (switches/lights). It exposes the Rflink2 command interface for these
    entities.

    The Rflink2 interface is managed as a class level and set during setup (and
    reset on reconnect).
    """
    ...
    @classmethod
    def set_rflink2_protocol(cls, protocol, wait_ack=None):
        """Set the Rflink2 asyncio protocol as a class variable."""
    ...
    def send_command(cls, device_id, action):
        """Send device command to Rflink2 and wait for acknowledgement."""
        return (yield from cls._protocol.send_command_ack(device_id, action))

    @asyncio.coroutine
    def _async_handle_command(self, command, *args):
        """Do bookkeeping for command, send it to rflink2 and update state."""
        ...
        if command == 'turn_on':
            ...
            # convert brightness to rflink2 dim level
    ...
    def cancel_queued_send_commands(self):
        ...
        queued for broadcast. Or when an incoming Rflink2 command (remote
    ...
    def _async_send_command(self, cmd, repetitions):
        """Send a command for device to Rflink2 gateway."""
        _LOGGER.debug(
            "Sending command: %s to Rflink2 device: %s", cmd, self._device_id)

        if not self.is_connected():
            raise HomeAssistantError('Cannot send command, not connected!')

        if self._wait_ack:
            # Puts command on outgoing buffer then waits for Rflink2 to confirm
            ...
            # Rflink2 protocol/transport handles asynchronous writing of buffer
...
class SwitchableRflink2Device(Rflink2Command):
    """Rflink2 entity which can switch on/off (eg: light, switch)."""

    def _handle_event(self, event):
        """Adjust state if Rflink2 picks up a remote command for this device."""
        self.cancel_queued_send_commands()
